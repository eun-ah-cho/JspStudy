...
<%
	...
	//파일업로드 처리용 코드 추가 
	String filename="";
	//String realFolder= "C:\\jsp-workspace\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\BookMarket\\resources\\images";//미리 폴더 경로설정 해놓기
	String realFolder= application.getRealPath("/resources/images");  //톰캣이 관리하는 실제경로 
	//http://192.168.111.101:8080/BookMarket 
	
	
	int maxSize = 5 * 1024 * 1024; //5메가까지 저장 
	String encType = "utf-8";  //파일명이 한글저장 
	MultipartRequest multipartRequest = new MultipartRequest(request, realFolder, maxSize, encType, new DefaultFileRenamePolicy()); 
	//만약 오류발생시 cos.jar 버전 확인 
  ...
  //String bookId = request.getParameter("bookId");
	...
	//다중파일용 파일명 가져오기!!
	Enumeration files = multipartRequest.getFileNames(); // 업로드 된 파일명들~ 을가져옴 
